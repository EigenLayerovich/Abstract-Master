...
def get_private_key(account: Accounts | dict, create_db: bool = False, evm_pk=True) -> str | int:
    """
    ...
    try:
        if evm_pk:
            ...
            privy_pk = account.privy_pk if isinstance(account, Accounts) else account['privy_pk']
            
            # Если включено шифрование, расшифровываем ключ
            if USE_PRIVATE_KEYS_ENCRYPTION:
                return CIPHER_SUITE[0].decrypt(privy_pk).decode()
...
def get_encrypted_pk(pk: str):
    if USE_PRIVATE_KEYS_ENCRYPTION:
        return CIPHER_SUITE[0].encrypt(pk.encode()).decode()
    return pk
